...
class SiteController extends Controller
{
	...
	public function actionActivate(){
		...
		if($result["is_active"] != 1){
			...
			if($result != 1){
				...
				$this->dao->commit();
				Helper::interrupt($this,"您的7663VR战网通行证已激活成功！",1,Yii::app()->url->getLoginUrl(),"","登录战网","您的通行证名(注册邮箱)<br/>".$username."<br/><br/>");
			}
		}else{
	...
	public function actionFindPwd(){
		...
		if($result == 1){
			...
			Helper::interrupt($this,"已将重置密码的链接发放至您的邮箱中，请登录后点击链接进行密码重置。",1,Yii::app()->url->getLoginUrl(),"","登录战网");
		}else{
	...
	public function actionDoRegister(){
		...
		if($result == false){		//确认用户有无被使用过
			...
			if($result > 0){
				...
				Helper::interrupt($this,"系统已将您的7663VR战网通行证验证发送至您的邮箱，请登录邮箱以后进行激活。！",1,Yii::app()->url->getRegisterUrl(),"","登录战网","您的通行证名(注册邮箱)<br/>".$data["username"]."<br/><br/>");
